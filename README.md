...
- Campo `abs` (abstención % por barrio, pestaña "Abstencionistas" del mapa). El potencial por puesto **no es dato abierto** (Resol. 4601/4602, solo entregado a partidos), así que se reconstruye (**Ruta B**): `abs = 1 − sufragantes / (nº de mesas × 360)`, donde sufragantes = votación total real por puesto (escrutinio) y `360` es el tope de ciudadanos por mesa en capitales, calibrado para que el censo de ciudad cuadre con el oficial de Bogotá presidencial 2026 (6.076.599 → cap efectivo 354/mesa). Abstención de ciudad resultante: 32,1%. Es una **estimación** precisa a ±1–3 pts a nivel barrio; unos pocos "puestos censo" especiales (Escuela Militar, Corferias) quedan topados a 80%. Para abstención oficial exacta, pasar el archivo de potencial por mesa de la carpeta "Básicos auditores presidente 2026" y reemplazar el denominador en `load_turnout`.
